...
export function useCreateCommentMutation() {
  ...
  return useMutation({
    ...
    onSuccess: async ({ comment }) => {
      // just do this 1 line ===> queryClient.invalidateQueries({queryKey: ["comments"]}) 
      // ....but it will starts refetch whole list of comments when all comments are open and we add new comment to it or if we return queryClient.invalidateQueries({queryKey: ["comments"]}), then also it will be in pending state for a while when user post new comment and it is a bad user experience.
